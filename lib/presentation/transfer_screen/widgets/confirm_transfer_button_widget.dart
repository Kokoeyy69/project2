...
import '../../security/pin_verification_screen.dart';
...
class _ConfirmTransferButtonWidgetState
  ...
  Future<void> _confirm() async {
    ...
    if (isVerified != true) {
      return; // Hentikan eksekusi jika user batal atau gagal verifikasi
    }
    ...
    try {
      ...
      final senderBalance =
          (userDoc.data()?[_balanceField] as num?)?.toDouble() ?? 0.0;
   ...
   Future<bool> _verifyPinGate() async {
     // Show PinVerificationScreen for PIN verification
     final bool? isVerified = await Navigator.push(
       context,
       MaterialPageRoute(builder: (context) => const PinVerificationScreen()),
     );
     ...
     if (isVerified != true) {
       if (mounted) {
         ScaffoldMessenger.of(context).showSnackBar(
           SnackBar(
             content: Text(
               'Transfer dibatalkan - verifikasi gagal',
   ...
   // TASK 3: 2FA Integration - Verify TOTP code if 2FA is enabled
   Future<bool> _verify2FAGate() async {
     final securityService = locator<SecurityService>();
     final is2FAEnabled = await securityService.getTwoFactorEnabled();

     if (!is2FAEnabled) {
       debugPrint('[ConfirmTransferButton] 2FA not enabled, skipping verification');
       return true;
     }

     debugPrint('[ConfirmTransferButton] 2FA enabled, requesting TOTP verification');

     final secret = await securityService.get2FASecret();
     if (secret == null || secret.isEmpty) {
       debugPrint('[ConfirmTransferButton] 2FA secret not found');
       if (mounted) {
         ScaffoldMessenger.of(context).showSnackBar(
           SnackBar(
             content: Text(
     ...
     final totpCode = await _show2FADialog();
     if (totpCode == null || totpCode.isEmpty) {
       debugPrint('[ConfirmTransferButton] 2FA verification cancelled');
       return false;
     }

     // Verify TOTP code
     final isValid = await securityService.verifyTOTP(secret, totpCode);
     if (isValid != true) {
       debugPrint('[ConfirmTransferButton] Invalid TOTP code');
       if (mounted) {
         ScaffoldMessenger.of(context).showSnackBar(
           SnackBar(
             content: Text(
     ...
     debugPrint('[ConfirmTransferButton] 2FA verification successful');
     return true;
   }
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Please select a different recipient to transfer to.',
              style: GoogleFonts.inter(
                fontSize: 13,
                color: AppTheme.textSecondary,
                height: 1.5,
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              isAmountZero ? 'Invalid Amount' : 'Complete Transfer Details',
              style: GoogleFonts.inter(
                fontSize: 16,
                fontWeight: FontWeight.w700,
                color: AppTheme.textPrimary,
                  ...
                  Expanded(
                    child: ElevatedButton(
                       onPressed: () async {
                         ...
                         // TASK 3: 2FA Verification Gate - Verify 2FA if enabled
                         final twoFAVerified = await _verify2FAGate();
